...
JO_X = function(nbar, S, cd, co, rho, eta) {
  ...
  uniroot(
    f = function(X) {
      X - (x * genhypergeo(
        U = c(1, 1, 1 + nbar * x + 1 / X),
        L = c(2, 2 + nbar * x),
        z = rho / eta * X,
        check_mod = FALSE
      )) /
        ((1 + nbar * x) * genhypergeo(
          U = c(1, 1, 1 + nbar * x + 1 / X),
          L = c(2, 1 + nbar * x),
          z = rho / eta * X,
          check_mod = FALSE
...
HypergeoSeriesTerm = function(k,
                              X,
                              nbar = 100,
                              x = S * co / (cd - co)) {
  exp(2 * logPoch(z = 1, n = k) -
        logPoch(z = 2, n = k) +
        k * log(nbar * X) -
        lgamma(1 + k)) * PochRatio(X = X, z = 1 + x * nbar, n = k)
}

HypergeoSeriesApprox = function(k, X, nbar, x)
  sum(HypergeoSeriesTerm(0:k, X, nbar, x))

JO_pmf = function(nvec,
                  X = NULL,
                  nbar,
                  S,
                  cd,
  ...
  if (is.null(X))
    X = x / (1 + x) / nbar
  if (mode == 'exact') {
    normalization = 1 / (rho / eta * X * genhypergeo(
      U = c(1, 1, 1 + nbar * x + 1 / X),
      L = c(2, 1 + nbar * x),
      z = rho / eta * X,
      check_mod = FALSE
  ...
  if (mode == 'approx') {
    normalization = 1 / (rho / eta * X * HypergeoSeriesApprox(
      k = 5e4,
      X = X,
      nbar = nbar,
      x = x
  ...
  f = 1 / nvec * (X * rho / eta) ^ nvec * PochRatio(X = X,
                                                    z = 1 + x * nbar,
                                                    n = nvec - 1)
...
JO_pmf_ratio = function(X, n1, n2, x, nbar, rho, eta) {
  if (n2 >= n1)
    return(NA)
  n2 / n1 * PochRatio(X, 1 + x * nbar + n2, n1 - n2) * (rho * X / eta) ^
    (n1 - n2)
}
...
Likelihood = function(X, data, nbar, S, cd, co, rho, eta, mode)
  sum(log(JO_pmf(data, X, nbar, S, cd, co, rho, eta, mode)))
...
## Normalization
Norm = function(E, sigma2, mu, nmax = 5e4)
  1 / sum(1 / seq(nmax) / (1 + 2 * E / sigma2 * seq(nmax)) ^ (1 + mu / E))

## Probability mass function
KeSh_pmf = function(n, E, b, d, nmax = 5e4) {
  ## d=eta, b=epsilon*rho*NR/NS
  sigma2 = b + d
  mu = b - d
