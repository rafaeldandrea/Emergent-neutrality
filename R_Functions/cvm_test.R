cvm_test <- function (x, y, type = c("W2", "U2", "A2"), simulate.p.value = FALSE, B = 2000, tol = 1e-08) {
  ...
  cvm.stat.disc <- function(x, y, type = c("W2", "U2", "A2")) {
    ...
    M <- switch(
            type, 
            W2 = E, 
            U2 = (diag(1, nrow(E)) - E %*% One %*% t(One)) %*% E %*% (diag(1, nrow(E)) - One %*% t(One) %*% E), 
            A2 = E %*% K
          )
    lambda <- eigen(M %*% Sy)$values
    STAT <- switch(type, W2 = sum(Z^2 * t)/N, U2 = sum((Z - Zbar)^2 * t)/N, A2 = sum((Z^2 * t/(H * (1 - H)))[-length(I)])/N)
  ...
  if (is.stepfun(y)) {
    ...
    STAT <- tempout[1]
    if(abs(Im(STAT)) < 1e-10) STAT <- Re(STAT)
    lambda <- tempout[2:length(tempout)]
    if(all(abs(Im(lambda)) < 1e-10)) lambda <- Re(lambda)
    if (!simulate.p.value) {
