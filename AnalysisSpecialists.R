  ...
  for (thecase in 2:7) {
    ...
    if (thecase <= 7) {
      ...
      if (thecase == 2) {
        ...
        dat2 = do.call('c', lapply(lf, function(l)
          get(load(l))))
        dat = c(dat, dat2)
      }
    ...
    if (thecase %in% 8:10) {
      ...
      raw_consumers <-
        ddply(tibble(name = lf), .(name), function(.) {
          ldply(get(load(.$name)), function(v)
...
sad_fit <-
  ...
  filter(mu == 13) %>%
  ddply(.(mu), function(.)
    fitSAD(., dbn = 'LS', method = fitting_test)$dtf) %>%
  ungroup %>%
  as_tibble %>%
...
Cd_thresh <-
  ...
  ldply(function(mod) {
    ...
    cd.threshold = (log(significance_cutoff / (1 - significance_cutoff)) -
                      intercept) / slope
    c(
      intercept = intercept,
      slope = slope,
      cd.threshold = cd.threshold,
      s_cd.threshold = abs(1 / slope) * sqrt((s_intercept) ^ 2 + (cd.threshold *
                                                                    s_slope) ^ 2)
    )
  }) %>%
...
df_logit_fit <-
  tibble(
    ...
    predLS = unlist(llply(logit, function(l)
      as.numeric(
        predict(l, data.frame(mu = themus), type = 'response')
      )))
  )
...
pE <- {
  results_summary %>%
    filter(NS == 50) %>%
    ggplot(aes(mu, mean)) +
    geom_line(aes(mus, predLS),
              data = df_logit_fit %>% filter(NS == 50),
              color = blue) +
...
p2b <- {
  Cd_thresh %>%
    ...
    annotate('text',
             x = 4.2,
             y = 1.1,
             label = paste('R2 = ', round(summary(model_log)$adj.r.squared, 2))) +
    annotate(
      'text',
      x = 4.2,
      y = 1.05,
      label = paste('slope =', model_log_slope, '\u00B1', model_log_se)
    ) +
    ggtitle('C')
}
...
if (FALSE) {
  ...
  pFall <-
    ...
    ggplot(aes(NS, cd.threshold, group = `Goodness-of-fit test`, color =
                 `Goodness-of-fit test`)) +
    geom_smooth(aes(fill = `Goodness-of-fit test`),
                method = 'lm',
                se = TRUE) +
    geom_point() +
    theme_bw() +
