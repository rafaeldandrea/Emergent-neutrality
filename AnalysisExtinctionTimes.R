...
if(datadir=='20190307'){
  ...
  scenarios <-
    expand.grid(
      limit=c('Generalists','ConstrainedBudget'),        ## equalize resource budget of all consumers?
      parametrization=c('FixedRates','FixedAbuns'),      ## fix rates eta, rho, or equil abuns N*, R*?
      bounce=TRUE,                                       ## do consumers immediately bounce from extinction?
      epsilon=1,                                         ## consumer conversion efficiency
      nu=0,                                              ## speciation rate
      NR=50,                                             ## number of resources
      NS=50,                                             ## number of consumers
      Rstar=100,                                         ## expected resource abun
      Nstar=Nstars,                                      ## expected consumer abun
      mu=seq(.05,2,l=5),                                 ## mean consumption rate in C matrix
      b=seq(0,1,l=11),                                   ## semirange of C matrix 
    ...
    filter(b>0) %>%
    filter(limit=='Generalists' | (limit=='ConstrainedBudget' & Nstar==100)) %>%
    filter(parametrization=='FixedAbuns' | !(Nstar>250 & Nstar<1800)) %>%
    mutate(index=seq(n())) %>%
    as_tibble
}
if(datadir=='20190514'){
  ...
  scenarios <-
    tibble(
      limit='Neutral',        ## equalize resource budget of all consumers?
      parametrization='FixedAbuns',      ## fix rates eta, rho, or equil abuns N*, R*?
      bounce=TRUE,                                       ## do consumers immediately bounce from extinction?
      epsilon=1,                                         ## consumer conversion efficiency
      nu=0,                                              ## speciation rate
      NR=50,                                             ## number of resources
      NS=50,                                             ## number of consumers
      Rstar=100,                                         ## expected resource abun
      Nstar=500,                                      ## expected consumer abun
      mu=.05,                                 ## mean consumption rate in C matrix
      b=0,                                   ## semirange of C matrix 
...
if(datadir=='20190520'){
  ...
  scenarios <-
      crossing(
        limit='Generalists',                      ## equalize resource budget of all consumers?
        parametrization='FixedAbuns',             ## fix rates eta, rho, or equil abuns N*, R*?
        bounce=TRUE,                              ## do consumers immediately bounce from extinction?
        epsilon=1,                                ## consumer conversion efficiency
        nu=0,                                     ## speciation rate
...
if(datadir=='20190530'){
  ...
  scenarios <- 
    crossing(
      limit='Specialists',                      ## equalize resource budget of all consumers?
      parametrization='FixedAbuns',             ## fix rates eta, rho, or equil abuns N*, R*?
      bounce=TRUE,                              ## do consumers immediately bounce from extinction?
      epsilon=1,                                ## consumer conversion efficiency
      nu=0,                                     ## speciation rate
  ...
  if(sample_all){
    dtf <-
      ...
      ddply(.(index),function(.){
        char=.$index
        print(char)
        dat=get(load(paste0('~/emergentneutrality/data/',datadir,'/ExtinctionTimes',char,'.Rdata')))
        pardat=get(load(paste0('~/emergentneutrality/data/',datadir,'/',char,'.Rdata')))[[1]]
        pardtf <-
          tibble(
      ...
      mutate(bmu=if(datadir!='20190530') round(b/mu,1) else mu) %>%
      as_tibble
  }
  ...
  if(sample_correction){
    dtf <-
      ...
      ddply(.(index),function(.){
        ...
        dat=get(
          load(
            paste0('~/emergentneutrality/data/',datadir,'/ExtinctionTimes',char,'.Rdata')
            )
          ) %>%
          mutate(
            window=clocktime%/%1800,
            group=findInterval(abundance,vec=exp(seq(log(1),log(max(1+abundance)))))
          ) %>% 
        ...
        unbiased_dat <-
          ...
          ddply(.(window),function(df){
            ddply(subset(df,window_permanent==1),.(species),function(.){
              y=subset(dat,window>unique(.$window) & species==unique(.$species))
...
dtf %>%
  ...
  ylab(expression(paste('slope of lm(extinction time ~ ',N[0],' / ',eta,')')))


v=uniroot(function(v) -1/log(v)*(1-v)/v-unique(dtf$Nstar),interval=c(1e-10,1-1e-10))$root
foo=tibble(
  n0=seq(max(dtf$n0)),
...
dtf_new <-
  dtf %>%
  mutate(
    eta=if(!datadir %in% c('20190507','20190530'))
      { 
        epsilon*Rstar*NR*mu 
      }else{ 
...
dtf_groups <-
  dtf_new %>%
  group_by(mu,bmu) %>%
  mutate(group=findInterval(n0,vec=base^(seq(log(min(n0,base)),log(max(1+n0,base)),l=15)))) %>%
  ungroup %>%
  group_by(mu,bmu,group) %>%
  summarize_at(
    c('n0','T','biotime','eta','clocktime'),
...
p1 <-
  ...
  geom_errorbar(
    aes(
      ymin=biotime_mean-biotime_se,
      ymax=biotime_mean+biotime_se
    ),
    width=0,
    color=red
  ) +
  geom_errorbarh(aes(xmin=n0_mean-n0_se,xmax=n0_mean+n0_se),color=red) +
...
p2 <-
  ...
  filter(mu>=1.1 | bmu==1) %>%
  mutate(bmu=round(bmu,1)) %>%
  ggplot(aes(T_mean,biotime_mean,group=as.factor(mu),color=as.factor(mu))) +
  geom_errorbar(
    aes(ymin=biotime_mean-biotime_se,ymax=biotime_mean+biotime_se),
    width=0,
    color='grey'
  ) +
  geom_errorbarh(aes(xmin=T_mean-T_se,xmax=T_mean+T_se),color='grey') +
...
thexlab=if(datadir!='20190530') expression(paste('b /',mu)) else expression(C[d]/C[o])
...
res2 %>%
  ggplot(aes(bmu,slope)) +
  stat_smooth(method='lm') +
  geom_errorbar(aes(ymax=slope+1.96*se,ymin=slope-1.96*se),color=red,width=.02) +
  geom_point() +
  ...
  annotate(
    ...
    x=.25,
    y=2,
    label=paste('R2 =',round(summary(lm(slope~bmu,data=res2))$adj.r.squared,2))
  ) +
  theme_bw()
...
dtf_groups %>% 
  group_by(bmu) %>% 
  summarize(
    p.value=shapiro.test(summary(lm(biotime_mean~T_mean))$residuals)$p.value
  )

dtf_groups %>% 
  group_by(bmu) %>% 
  summarize(
    intercept=coef(summary(lm(biotime_mean~T_mean)))[1,1],
    intercept_se=coef(summary(lm(biotime_mean~T_mean)))[1,2],
    slope=coef(summary(lm(biotime_mean~T_mean)))[2,1],
    slope_se=coef(summary(lm(biotime_mean~T_mean)))[2,2]
  )
...
p3 <-
  ...
  group_by(bmu) %>% 
  summarize(
    Chi2=sum((biotime_mean-T_mean)^2/T_mean),
...
pAD <-
  dtf_groups %>%
  mutate(
    bmu=if(datadir %in% c('20190507','20190530')) round(bmu,1) else round(bmu/sqrt(3),2)
  ) %>%
  filter(bmu %in% range(bmu)) %>%
  ggplot(aes(n0_mean,biotime_mean*eta_mean,group=factor(bmu),color=factor(bmu))) + 
  labs(color=ifelse(datadir %in% c('20190507','20190530'),expression(C[d]/C[o]),'CV')) +
  geom_errorbar(
    aes(
  ...
  geom_errorbarh(aes(xmin=n0_mean-n0_se,xmax=n0_mean+n0_se,color=factor(bmu))) +
...
pBE <-
  ...
  ggplot(aes(T_mean*eta_mean,biotime_mean*eta_mean,color=factor(bmu),group=factor(bmu))) +
  labs(color=ifelse(datadir %in% c('20190530','20190507'),expression(C[d]/C[o]),'CV')) +
  geom_errorbar(
    aes(
      ymin=(biotime_mean-biotime_se)*eta_mean,
      ymax=(biotime_mean+biotime_se)*eta_mean,
...
if(do.pdf){
  ...
  ggsave(
    ...
    plot=gridExtra::grid.arrange(
      pA + scale_x_log10() + scale_y_log10(),pB, pC,
      pD + scale_x_log10() + scale_y_log10(),pE, pF,
      ncol=3,
      respect=TRUE
    ),
    width=27,
